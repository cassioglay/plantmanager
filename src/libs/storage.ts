...
export async function savePlant(plant: PlantProps): Promise<void> {
	try {
		...
		if (repeat_every == 'week') {
			...
			nexTime.setDate(nexTime.getTime() + 1);

		}

		const seconds = Math.abs(Math.ceil(now.getTime() - nexTime.getTime()) / 1000);

		const notificationId = await Notications.scheduleNotificationAsync({
			content: {
				title: "Heeey, 🌱",
				body: `Está na hora de cuidar da sua ${plant.name}`,
				sound: true,
				priority: Notications.AndroidNotificationPriority.HIGH,
				data: {
					plant
				},
			},
...
export async function loadPlant(): Promise<PlantProps[] | any> {
	try {
		const data = await AsyncStorage.getItem("@plantmanager:plants");
...
export async function removePlant(id: string): Promise<void> {
	...
	await Notications.cancelScheduledNotificationAsync(plants[id].notificationId);
